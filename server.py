...
def parseImageNormMidCoords(img_path: str, model) -> tuple[bool, dict | None]:
    try:
...
async def api_get_3d_distances(
    ...
    try:
        ...
        error, norm_coords = parseImageNormMidCoords(img_temp, model)
...
async def get_tim_dist(
    ...
    try:
        ...
        model = request.app.state.model
        error, coords_data = parseImageNormMidCoords(temp_img_path, model)
